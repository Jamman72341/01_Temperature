...
class Colour_chart:
    def __init__(self, parent):
        ...
        self.frame = Frame(parent, height=500)
        self.frame.pack()

        self.canvas = Canvas(self.frame, bg='#FFFFFF', width=1100,
                             height=500, scrollregion=(0, 0, 2500, 1600))
        ...
        for i in range(len(COLORS)):
            lbl = Label(self.colour_blocks, bg=COLORS[i],
                        text=COLORS[i], fg="black",
                        padx=10, pady=10, width=12)
            lbl.grid(row=i // NUM_COL, column=i % NUM_COL)


# Main Routine
root = Tk()
show_chart = Colour_chart(root)
root.mainloop()
# root.destroy()